...
// Or you can use dynamic import() if it's available
// const emailModule = import('emailjs');
// emailModule.then((module) => {
//   email = module;
// }).catch((err) => {
//   // Handle the error if necessary
// });

// Then use `email` variable wherever necessary in the module

/**
 * @constructs Mail
 * @param {object} options Hash of options.
 */
const Mail = exports.Mail = async function(options) {
  options = options || {};
  ...
  // Dynamically import emailjs module
 ...
 *              winston >= 3 undefined
 */
Mail.prototype.log = async function (arg0, arg1) {
  // get winston version to create tests accordingly
  const winstonVersion = winston.version;
  const majorWVersion = winstonVersion.split('.')[0];
  const self = this;

  let callback, level, msg, meta, info;
  ...
  const message = email.message.create(msgOptions);
